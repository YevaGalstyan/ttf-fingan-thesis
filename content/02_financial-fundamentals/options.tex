...
The relation between the current price and $K$ is described by three terms. A call option is \textbf{in the money} if $F_t > K$, \textbf{at the money} if $F_t = K$ and \textbf{out of the money} if $F_t < K$, where $F_t$ is the settlement price on trading day $t$. For a put option, the definitions are reversed \parencite[248]{hullOptionsFuturesOther2022}. Moneyness of an option is the ratio of the strike price to the current price, $K / F_t$, so a call is in the money when $K / F_t < 1$.
