...
\end{equation}
where $F_t$ is the settlement price on the valuation date, $\hat{x}_k$ is the simulated return on step $k$, and $h$ is the number of trading days to expiry. The rollout was repeated with independent noise vectors to give a distribution of $\hat{F}_T$. The expected payoff of the option was calculated following the Monte Carlo procedure described in Sec.~\ref{sec:options}. Monte Carlo standard error was computed for the expected payoff. The error is the standard deviation of the payoff divided by $\sqrt{M}$, where $M=20000$ is the number of simulation paths.
