...
\begin{table}[H]
  ...
  \begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}rrrr}
    ...
    \bottomrule
  \end{tabular*}
  \caption{Distributional statistics and autocorrelation of generated daily TTF log returns on the test split, the combinations without the $\mathrm{SR}^{*}$ term, configuration 8}
...
\begin{table}[H]
  ...
  \begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}rrrr}
    ...
    \bottomrule
  \end{tabular*}
  \caption{Distributional statistics and autocorrelation of generated daily TTF log returns on the test split, the combinations with the $\mathrm{SR}^{*}$ term, configuration 8}
  \label{tab:loss_c9_sr}
\end{table}

\textbf{Asymmetry.} The seed ranges of all four branches containing the $\mathrm{SR}^{*}$ term are separable from ForGAN on skewness, and all four are negative (Fig.~\ref{fig:loss_skew_max}). The skewness is \ms{-0.59}{0.16} for the $\mathrm{SR}^{*}$ branch, and the three branches containing $\mathrm{SR}^{*}$ are within \num{0.05} of it, against the \ms{0.54}{0.88} of the ForGAN baseline and the \num{0.92} of the test split.  The mean skewness of the $\mathrm{PnL}^{*}_{a}$ \& $\mathrm{STD}$ branch is \ms{-0.09}{0.95}, but its seed range crosses zero and overlaps the ForGAN baseline. The highest skewness is \ms{1.11}{0.42} for the $\mathrm{PnL}^{*}_{a}$ \& $\mathrm{MSE}$ \& $\mathrm{STD}$ branch. The seed ranges of the ForGAN baseline
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/loss_skew_max.pdf}
  \caption{Skewness and maximum of the generated returns, the ten cost function
  branches, configuration 8}
  \label{fig:loss_skew_max}
\end{figure}
...
The seed ranges of the $\mathrm{PnL}^{*}_{a}$ branch contain the test split on the skewness, the maximum and the \SI{1}{\percent} quantile, as does the ForGAN baseline. $\mathrm{SR}^{*}$ compresses the largest generated returns, which brings the \SI{99}{\percent} quantile closer to the test split but moves the skewness and the maximum away.
