...
This chapter reports the experimental configurations and results for the TTF return series. It covers the properties of the realized series alongside the ForGAN baseline across different training lengths, capacities, condition windows and input scaling. Next, the chapter examines the effect of cost function terms, followed by model selection and option prices. All configurations are trained with five fixed seeds, and the values are reported as mean and seed spread. The seed range of a value is its mean plus or minus one seed spread. The difference between configurations or cost function branches is treated as separable if their seed ranges do not overlap. A generated value is treated as separable from the test split if its seed range does not contain the realized value. A configuration or cost function is treated as closer to the test split if its seed ranges are narrower and contain the realized value on more statistics. Each subsection reports a defined slice of the full experimental grid from App.~\ref{app:grid}.
